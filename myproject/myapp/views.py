...
def checkForUpdates(request):
    ...
    logger.debug("User " + username + " synched")
...
def pull_file(request):
    ...
    logger.debug("File Pulled. Filename:" + filename)
    return httpresponse
...
def login(request):
    if request.method == "POST":
        ...
        if (user is not None):
            ...
            logger.debug("Improper Login by username:" + username)
    ...
    if request.method == 'POST':
        if request.user.is_authenticated:
            ...
            for f in docsByThisUser:
                ...
                # f.delete()
            print "---"
            logger.debug("User " + username + " updated files")
...
def get_list_files(request):
    ...
    logger.debug("List of files view by user " + username)
